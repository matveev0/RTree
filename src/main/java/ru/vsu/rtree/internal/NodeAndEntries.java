...
public final class NodeAndEntries<T, S extends Geometry> {
     ...
     * Constructor.
     *
     * @param node         absent = whole node was deleted
     *                     present = either an unchanged node because of no removal or
     *                     the newly created node without the deleted entry
     * @param entries      from nodes that dropped below minChildren in size and thus
     *                     their entries are to be redistributed (readded to the tree)
     * @param countDeleted count of the number of entries removed
     */
